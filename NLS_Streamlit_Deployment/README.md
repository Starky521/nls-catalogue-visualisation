...
All three pages are generated from Python and Plotly, so their text can be
edited directly on GitHub. The website reads processed CSV files rather than
the five-million-row master Parquet dataset.

See `EDIT_TEXT_GUIDE.md` for the exact files to edit when changing titles,
introductions and captions directly on GitHub.
